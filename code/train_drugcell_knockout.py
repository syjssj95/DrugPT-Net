...
import util
from util import *
from drugcell_NN_knockout import *
...
def train_model(root, term_size_map, term_direct_gene_map, dG, train_data, gene_dim, drug_dim, model_save_folder, train_epochs, batch_size, learning_rate, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, cell_features, drug_features, gamma, gene_embed_dim, perturb_table, node_num, topN_else_mask, gene_embed_hidden_dim, gene2id_mapping):
    ...
    knockout_lst = ['ABCB6', 'ABHD6', 'GO:0007006']
    
    createFolder(model_save_folder)
    
    # dcell neural network
    model = drugcell_nn_knockout(term_size_map, term_direct_gene_map, dG, gene_dim, drug_dim, root, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, gamma, gene_embed_dim, topN_else_mask, gene_embed_hidden_dim, gene2id_mapping)
    ...
    for epoch in range(train_epochs):
        ...
        with tqdm(train_loader, unit='batch') as tepoch:
            for i, (inputdata, labels) in enumerate(tepoch):
                ...
                aux_out_map, _ = model(cuda_features, perturb_table, knockout_lst)
...
train_model(root, term_size_map, term_direct_gene_map, dG, train_data, num_genes, drug_dim, model_dir, opt.epoch, opt.batchsize, opt.lr, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, cell_features, drug_features, gamma, gene_embed_dim, perturb_table, node_num, opt.topN_else_mask, opt.gene_embed_hidden_dim, gene2id_mapping)
