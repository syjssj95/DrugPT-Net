...
class drugcell_nn_no_netgp(nn.Module):
    ...
    def __init__(self, term_size_map, term_direct_gene_map, dG, ngene, ndrug, root, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, gamma, gene_embed_dim, topN_else_mask, gene_embed_hidden_dim):
        ...
        super(drugcell_nn_no_netgp, self).__init__()
...
#         self.add_module('root_layer_output', nn.Linear(1,1))
    ...
    def construct_NN_gene(self):
        input_size = self.gene_dim

        for i in range(len(self.num_hiddens_drug)):
            self.add_module('gene_linear_layer_' + str(i+1), nn.Linear(input_size, self.num_hiddens_drug[i]))
            self.add_module('gene_batchnorm_layer_' + str(i+1), nn.BatchNorm1d(self.gene_embed_dim))
#             self.add_module('drug_aux_linear_layer1_' + str(i+1), nn.Linear(self.num_hiddens_drug[i],1))
#             self.add_module('drug_aux_linear_layer2_' + str(i+1), nn.Linear(1,1))
            input_size = self.num_hiddens_drug[i]
...
#         gene_out = gene_input_final_permute
#         for i in range(1, len(self.num_hiddens_drug)+1, 1):
#             gene_out = self._modules['gene_batchnorm_layer_'+str(i)]( torch.tanh(self._modules['gene_linear_layer_' + str(i)](gene_out)))
#         gene_out = gene_out.permute(0, 2, 1)
        term_gene_out_map = {}
...
#         final_value = self._modules['final_linear_layer_output'](aux_layer_out)
