...
class drugcell_nn(nn.Module):
    ...
    def __init__(self, term_size_map, term_direct_gene_map, dG, ngene, ndrug, root, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, gamma, gene_embed_dim, topN_else_mask, gene_embed_hidden_dim):
...
#                 print(f'{term}_batchnorm_layer: {term_NN_out_map[term].shape}')
                ...
                # learning layer for embedded vector dimesion in each gene
#                 term_batchnorm_out = self._modules[term+'_batchnorm_layer'](Tanh_out)

#                 term_gene_embed1 = torch.tanh(self._modules[term+'_gene_vector_linear_layer1'](term_batchnorm_out)).permute(0, 2, 1)
#                 term_gene_batchnorm1 = self._modules[term+'_gene_vector_batchnorm_layer1'](term_gene_embed1).permute(0, 2, 1)
#                 term_gene_embed2 = torch.tanh(self._modules[term+'_gene_vector_linear_layer2'](term_gene_batchnorm1)).permute(0, 2, 1)
#                 term_gene_batchnorm2 = self._modules[term+'_gene_vector_batchnorm_layer2'](term_gene_embed2).permute(0, 2, 1)
#                 print('vector embedding 이후 shape:', term_gene_batchnorm2.shape)
#                 term_NN_out_map[term] = torch.add(term_batchnorm_out, term_gene_batchnorm2)
