...
def predict_dcell(predict_data, gene_dim, drug_dim, model_file, hidden_folder, batch_size, result_file, cell_features, drug_features, perturb_table, test_result_df, cv, CUDA_ID):
    ...
    with torch.no_grad():
        with tqdm(test_loader, unit='batch') as tstepoch:
            for i, (inputdata, labels) in enumerate(tstepoch):
                ...
                aux_out_map, term_hidden_map = model(cuda_cell_feature, cuda_drug_feature, cuda_drug_idx, perturb_table)
                ...
                for term, hidden_map in term_hidden_map.items():
                    ...
                    # with open(hidden_file, 'ab') as f:
                        # for my code
                        # data_3d = hidden_map.data.cpu().numpy()
                        # data_2d = data_3d.reshape(data_3d.shape[0], -1)
                        # np.savetxt(f, data_2d, '%.4e')
        ...
        #         with open(f'test_predict.pickle', 'wb') as f:
        #             pickle.dump(test_predict, f, pickle.HIGHEST_PROTOCOL)

        #         with open(f'test_tot_labels.pickle', 'wb') as f:
...
if __name__ == "__main__":  
    ...
    perturb_table = torch.from_numpy(np.genfromtxt('data/mydata/netGP_top100.tsv', delimiter='\t', skip_header=1)[:, 2:]).cuda(CUDA_ID)
    ...
    predict_dcell(predict_data, num_genes, drug_dim, load_dir[0], hidden_dir, opt.batchsize, result_dir, cell_features, drug_features, perturb_table, test_result_df, opt.cv, CUDA_ID)	
