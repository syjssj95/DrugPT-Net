...
# from drugcell_NN_no_netgp import *
...
def train_model(root, term_size_map, term_direct_gene_map, dG, train_data, gene_dim, drug_dim, model_save_folder, train_epochs, batch_size, learning_rate, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, cell_features, drug_features, gamma, gene_embed_dim, perturb_table, node_num, topN_else_mask, gene_embed_hidden_dim):
    ...
    model = drugcell_nn(term_size_map, term_direct_gene_map, dG, gene_dim, drug_dim, root, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, gamma, gene_embed_dim, topN_else_mask, gene_embed_hidden_dim)
    
    # model = drugcell_nn_no_netgp(term_size_map, term_direct_gene_map, dG, gene_dim, drug_dim, root, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, gamma, gene_embed_dim, topN_else_mask, gene_embed_hidden_dim)
...
#                 final_value = model(cuda_features, perturb_table)
#                 if train_predict.size()[0] == 0:
#                     train_predict = final_value
#                     tot_labels = cuda_labels
#                 else:
#                     train_predict = torch.cat([train_predict, final_value], dim=0)
#                     tot_labels = torch.cat([tot_labels, cuda_labels], dim=0)

#                 loss_train = loss(final_value, cuda_labels)
#                 total_loss_train += loss_train
...
#                     final_value = model(cuda_features, perturb_table)
#                     if test_predict.size()[0] == 0:
#                         test_predict = final_value
#                         test_tot_labels = cuda_labels
#                     else:
#                         test_predict = torch.cat([test_predict, final_value], dim=0)
#                         test_tot_labels = torch.cat([test_tot_labels, cuda_labels], dim=0)

#                     loss_val = loss(final_value, cuda_labels)
#                     total_loss_val += loss_val
...
# parser.add_argument('-fingerprint', type=str, default='drug2fingerprint_2048.txt')
...
# parser.add_argument('-netgp_score', type=str, default='1')
parser.add_argument('-gene_embed_hidden_dim', type=int, default=32)
...
netGP = pd.read_csv('data/mydata/netGP_profile.out', sep='\t')
netGP_col = netGP.columns[2:]
netGP[netGP_col] = netGP[netGP_col].apply(top_n_to_1, axis=1)

drug_target = pd.read_csv('data/mydata/drug_target_info.tsv', sep='\t')
...
# netgp_top = list(map(int, opt.netgp_top.split(',')))
# netgp_score = list(map(float, opt.netgp_score.split(',')))


# def top_n_to_score(row, top_lst, score_lst):
#     sorted_row = np.argsort(row)[::-1]
#     result_lst = np.zeros_like(row).astype(float)
#     for i in range(len(top_lst)):
#         if i ==0:
#             result_lst[sorted_row[:top_lst[i]]] = score_lst[i]
#         else:
#             result_lst[sorted_row[top_lst[i-1]:top_lst[i]]] = score_lst[i]
            
#     return(result_lst)

# netGP = pd.read_csv('data/mydata/netGP_profile.out', sep='\t').drop(columns=['drug_name', 'drug_idx'])
# netGP = netGP.apply(lambda row: top_n_to_score(row, netgp_top, netgp_score), axis=1)
# perturb_table = torch.tensor(netGP)
...
train_model(root, term_size_map, term_direct_gene_map, dG, train_data, num_genes, drug_dim, model_dir, opt.epoch, opt.batchsize, opt.lr, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, cell_features, drug_features, gamma, gene_embed_dim, perturb_table, node_num, opt.topN_else_mask, opt.gene_embed_hidden_dim)
